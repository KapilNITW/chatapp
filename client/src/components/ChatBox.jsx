...
const ChatBox = () => {
    ...
    if (error) {
        return (
            <p style={{ textAlign: "center", width: "100%" }}>
                Error loading recipient user: {error.message}
            </p>
        );
    }
    ...
    return (
        <Stack gap={4} className="chat-box">
            <div className="chat-header">
                <strong>{recipientUser.name || "No Recipient"}</strong>
            </div>
            ...
            <Stack direction="horizontal" gap={3} className="chat-input flex-grow-0">
                ...
                <button className="send-btn" onClick={()=>{
                    sendTextMessage(textMessage,user,currentChat._id,sendTextMessage) && setTextMessage("");
